        ...
        <div class="modal-content">
            <form id="createGroupForm">
                <div class="modal-header">
                    <h5 class="modal-title" id="createGroupLabel">Create New Group Chat</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div id="memberList" class="border rounded p-2" style="max-height:200px; overflow-y:auto;">
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-sm btn-success" disabled>Create Group Chat</button>
                    <a href="javascript:void(0);" class="btn btn-sm btn-primary" data-bs-toggle="modal"
                        data-bs-target="#chat_modal">Back To Chatbox</a>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade zoomIn" id="chat_modal" tabindex="-1" aria-labelledby="showUpdateLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body">
                <div class="d-lg-flex gap-4">
                    <div class="chat-leftsidebar">
                        <div class="px-2 pt-2 mb-4">
                            ...
                            <div class="tab-pane active" id="chats" role="tabpanel">
                                <div class="chat-room-list pt-3" data-simplebar="">
                                    ...
                                    <div class="chat-message-list">
                                        <ul class="list-unstyled chat-list chat-user-list mb-0" id="channelList"></ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="w-100 overflow-hidden position-relative">
                                <div class="position-relative">
                                    <div class="position-relative" id="users-chat">
                                                        ...
                                                        <li class="list-inline-item m-0">
                                                            <div class="dropdown">
                                                                <button
                                                                    ...
                                                                    type="button" data-bs-toggle="dropdown"
                                                                    aria-haspopup="true" aria-expanded="false">
                                                                    <iconify-icon icon="pepicons-pop:dots-y" width="25"
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-end">
                                                                    ...
                                                                    <a class="dropdown-item" href="#"><i
                                                                        ...
                                                                        Muted </a>
                                                                    <a class="dropdown-item" href="#" onclick="deleteChat()" id="id_for_delete_chat"><i
                                        ...
                                        <div class="chat-conversation" id="chat-conversation" data-simplebar="">
                                            <ul class="list-unstyled" id="users-conversation">
                                                <div id="defaultChatMessage" class="text-center"
                                                    ...
                                                    <p class="text-muted mt-1">Search a user to start a conversation...
                                                    </p>
                                                </div>
                                            </ul>
                                            <div id="typing-indicator" class="text-muted fst-italic px-3"
                                                style="display:none; font-size:12px; margin-bottom: 10px;">
                                                Someone is typing...
                                            </div>
                                        </div>
                                        <div id="attachments-preview" class="mt-2" style="display: none;">
                                            <div id="attachments-list" class="d-flex flex-wrap gap-2 mb-2"></div>
                                        </div>
...
<script>
    ...
    function getFileIcon(file) {
        ...
        if (["jpg", "jpeg", "png", "gif"].includes(ext)) return "bx bx-image";
        if (["pdf"].includes(ext)) return "bx bx-file-pdf";
        if (["doc", "docx"].includes(ext)) return "bx bx-file";
        if (["xls", "xlsx"].includes(ext)) return "bx bx-spreadsheet";
        return "bx bx-paperclip";
    }
    ...
    function updateAttachmentDisplay() {
        ...
        if (attachedFiles.length > 0) {
            ...
            attachedFiles.forEach((file, index) => {
                ...
                const li = $(`
                    <div class="border rounded p-1 d-flex align-items-center gap-2 bg-light shadow-sm" style="max-width: 250px;">
                        <i class="${icon} text-primary fs-16"></i>
                        <span class="small text-truncate flex-grow-1">${file.name}</span>
                        <button type="button" class="btn btn-sm btn-danger p-0 px-1 remove-single" data-index="${index}" title="Remove">
    ...
    $(document).on("click", ".remove-single", function () {
        ...
        $("#chat-attachment")[0].files = dt.files;

        toastr.warning("File removed.");
    });
</script>
<script type="module">
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#chatinput-form").submit(function (e) {
            ...
            if (group_id) {
                // Send group message
                $.ajax({
                    url: "<?php echo base_url('Chat/send_group_message'); ?>",
                    type: "POST",
                    data: { message: $("#chat-input").val(), group_id: group_id },
                    success: function (response) {
        ...
        <li id="contact-id-${user.emp_id}" class="chat-user-item" data-name="direct-message">
            <div class="d-flex align-items-center justify-content-between">
                <a href="javascript:void(0);" 
                    ...
                    onclick="openChat('${user.emp_id}', '${user.name}', '${user.photo}')">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1 ms-2" style="min-width:0;">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <span class="text-muted" style="font-size: 8px;">${formatTime(user.date_send)}</span>
                                ${unreadBadge}
                            </div>
                            
                            <p class="${lastMessageClass} mb-0 fs-10"
                ...
                <div class="dropdown ms-2">
                    <button class="btn p-0 border-0 bg-transparent" 
                        ...
                        <iconify-icon icon="pepicons-pencil:dots-y" width="16" height="16"></iconify-icon>
                    </button>

                    <ul class="dropdown-menu dropdown-menu-end shadow-sm">
                        <li>
                            <a class="dropdown-item text-danger" href="javascript:void(0);" onclick="deleteChat('${user.emp_id}')">
    ...
    function deleteChat(empId) {
        if (!empId) {
                empId = $("#id_for_delete_chat").val(); // Make sure this is the correct input ID
            }
            ...
            if (!empId) {
                Swal.fire({
                    ...
                    text: "Please select a user to delete the chat.",
                    icon: "error",
        ...
        Swal.fire({
            title: "Delete Chat?",
            text: "This will permanently remove your chat history with this user. As well as any media, links, and documents shared in the chat in both parties.",
            icon: "warning",
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    beforeSend: function () {
                        Swal.fire({
                            title: "Deleting...",
                            text: "Please wait while we remove the chat.",
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            $("#contact-id-" + empId).fadeOut(300, function () {
                                $(this).remove();
                            });
                            $("#contact-tab-id-" + empId).fadeOut(300, function () {
                                $(this).remove();
                            });
                            ...
                            $('#users-conversation').html(`
                                <div id="defaultChatMessage" class="text-center" style="margin-bottom: 180px;">
                                    <iconify-icon icon="fluent-color:chat-more-24" width="200" height="90"></iconify-icon>
                                    <p class="text-muted mt-1">Search a user to start a conversation...</p>
                                </div>
                            `);
                            $('#chat-input').val('');
                            $('#receiver_id').val('');
                            ...
                            $('#profile_photo').hide();
                            $('.username').text('');
                            $('#attachment-count').hide().text('0');
                            $('#typing-indicator').hide();
                            $("#attachments-preview").hide();
                            $("#attachments-list").empty();
                            $("#id_for_delete_chat").val('');

                            socket1.emit("conversationDeleted", {
                                sender_id: currentUsers,
                                receiver_id: empId
                            });
    ...
    socket1.on("conversationDeleted", function (data) {
        if (data.receiver_id == currentUsers) {
            $("#contact-id-" + data.sender_id).fadeOut(300, function () {
                $(this).remove();
            });
            $("#contact-tab-id-" + data.sender_id).fadeOut(300, function () {
                $(this).remove();
            });
            ...
            if ($("#receiver_id").val() == data.sender_id) {
                $("#users-conversation").html(
                    ...
                    The sender has deleted this conversation.
                </div>`
                );
            }

            toastr.warning("The sender deleted your conversation.");
        }

        if (data.sender_id == currentUsers) {
            $("#contact-id-" + data.receiver_id).fadeOut(300, function () {
                $(this).remove();
            });
            $("#contact-tab-id-" + data.receiver_id).fadeOut(300, function () {
                $(this).remove();
            });
            $("#users-conversation").html("");
            $("#receiver_id").val("");
            ...
            contactListHtml += `
                <li id="contact-tab-id-${user.emp_id}" class="contact-user-item">
                    <a href="javascript:void(0);" onclick="openChat('${user.emp_id}', '${user.name}', '${user.photo}')">
                        <div class="d-flex align-items-center">
                            <div class="flex-shrink-0 chat-user-img mt-1">
                                <img src="http://172.16.161.34:8080/hrms/${user.photo}" class="img-thumbnail avatar-xs rounded-circle" style="border-color: blue">
                            </div>
                            <div class="flex-grow-1 ms-2" style="min-width:0;">
                                <div class="d-flex justify-content-between align-items-center">
                                    <span class="fw-semibold fs-10">${user.name}</span>
    ...
    function openChat(receiver_id, userName, photo) {
        ...
        $("#id_for_delete_chat").val(receiver_id);
        ...
        $("#users-conversation").append(`
            <li class="chat-list ${alignment}">
                <div class="d-flex ${isMine ? "justify-content-end" : "justify-content-start"}">
                    ...
                    <div class="chat-bubble-container ${isMine ? "text-end" : "text-start"}" style="max-width: 40%;">
                        ${msgBubble}
    ...
    function addToChatList(receiver_id, userName, photo, last_message, last_time, attachments = [], sender_id = null) {
        ...
        let lastMessageClass = "last-message text-truncate mb-0";
        if (activeChat == receiver_id) {
            lastMessageClass += " text-muted text-truncate";
        } else {
            lastMessageClass += (sender_id != currentUsers) ? " fw-bold text-truncate" : " text-muted text-truncate";
        }
            ...
            $("#userList").append(`
                <li id="contact-id-${receiver_id}" class="chat-user-item" data-name="direct-message" data-contact-id="${receiver_id}">
                    <a href="javascript:void(0);" onclick="openChat('${receiver_id}', '${userName}', '${photo}')">
                        <div class="d-flex align-items-center">
                            <div class="flex-shrink-0 chat-user-img mt-1">
                                <img src="http://172.16.161.34:8080/hrms/${photo}" class="img-thumbnail avatar-xs rounded-circle" style="border-color: blue"> 
                            </div>
                            <div class="flex-grow-1">
    ...
    function loadMemberList() {
        $.ajax({
            url: "<?php echo base_url('Chat/get_users'); ?>",
            method: "GET",
            dataType: "json",
            success: function (users) {
                ...
                users.forEach(user => {
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" value="${user.emp_id}" id="member-${user.emp_id}">
                        <label class="form-check-label" for="member-${user.emp_id}">
                            <img src="http://172.16.161.34:8080/hrms/${user.photo || '/images/default_user.png'}" class="rounded-circle me-2" width="30" height="30">
                           <i> ${user.name}</i>
                        </label>
                    </div>
                `;
    ...
    $("#createGroupModal").on("show.bs.modal", loadMemberList);
    $("#createGroupModal").on("hide.bs.modal", function () {
        $("#group_name").val("");
    ...
    $("#createGroupForm").on("submit", function (e) {
        ...
        $.ajax({
            url: "<?php echo base_url('Chat/create_group'); ?>",
            method: "POST",
