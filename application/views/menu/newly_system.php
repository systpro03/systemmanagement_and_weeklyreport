                    ...
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <div>
                            ...
                            <small class="text-danger d-block fst-italic mt-1"> 
                                ...
                                Editing and deleting files is allowed only for users from the uploader's team.
                            </small>
                        </div>
                        <div class="d-flex align-items-center gap-2">
                            <?php if ($this->session->userdata('position') != 'Manager') { ?>
                                <button class="btn btn-primary create-folder-modal rounded-pill" data-bs-toggle="modal"
                ...
                <table id="folderTable" class="table table-striped table-hover compact" width="100%">
                    <thead class="table-dark">
                        <tr>
                            ...
                            <th width="10%" class="text-center">Filename</th>
...
<!-- File Upload Modal -->
<div class="modal fade zoomIn" id="file_upload" tabindex="-1" aria-labelledby="file_upload" data-bs-backdrop="static"
...
<script>
    ...
    function loadModule() {
        $.ajax({
            url: '<?php echo base_url('Menu/Newly_System/setup_module_new') ?>',
            type: 'POST',
    ...
    $.ajax({
        url: '<?php echo base_url('Menu/Newly_System/business_unit') ?>',
        type: 'POST',
        success: function (response) {
    ...
    function filter() {
        ...
        $.ajax({
            url: '<?php echo base_url('Menu/Newly_System/get_new_folders'); ?>',
            type: 'POST',
            data: {
                team: team,
                module: module,
...
<script>
    ...
    function updateFolderModalContent() {
        ...
        folderContentTimeout = setTimeout(function () {
            ...
            $('#folderTable').DataTable({
                ...
                stateSave: true,
                ajax: {
                    url: '<?php echo base_url("Menu/Newly_System/view_folder_modal_server"); ?>',
                    type: 'GET',
                    data: {
    ...
    $('#folderModal').on('shown.bs.modal', function () {
        ...
        $('#folderTable').DataTable().columns.adjust().draw();
    });
...
<script>
        ...
        document.querySelector('#uploadForm').addEventListener('submit', function (e) {
            ...
            Swal.fire({
                ...
                `;
                    ...
                    $.ajax({
                        url: '<?php echo base_url('Menu/Newly_System/upload_new_files'); ?>',
                        type: 'POST',
                        data: formData,
                        contentType: false,
                        processData: false,
    ...
    function deleteFile(folderName, fileName, module) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?php echo base_url('Menu/Newly_System/delete_file_new'); ?>',
                    type: 'POST',
                    data: {
                        folder_name: folderName,
                        file_name: fileName,
